...
/// <summary>
/// Scraper for Spirit rover using PDS (Planetary Data System) index files
/// Parses tab-delimited index files instead of querying JSON APIs
/// </summary>
public class SpiritScraper : IScraperService
{
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly MarsVistaDbContext _context;
    private readonly ILogger<SpiritScraper> _logger;
    private readonly PdsIndexParser _parser;

    private const int SpiritRoverId = 4;  // Database rover ID for Spirit
    ...
    public string RoverName => "Spirit";

    public SpiritScraper(
        IHttpClientFactory httpClientFactory,
        MarsVistaDbContext context,
        ILogger<SpiritScraper> logger,
    ...
    /// <summary>
    /// Scrape latest photos (not applicable for Spirit - mission complete)
    /// </summary>
    public async Task<int> ScrapeAsync(CancellationToken cancellationToken = default)
    {
        _logger.LogInformation("ScrapeAsync not applicable for Spirit (use ScrapeVolumeAsync or ScrapeAllVolumesAsync)");
        return 0;
    }
    ...
    {
        _logger.LogInformation("ScrapeSolAsync not implemented for Spirit (use ScrapeVolumeAsync)");
        return 0;
    }

    /// <summary>
    /// Scrape a single PDS volume (e.g., mer2ps_0xxx for PANCAM)
    /// </summary>
    public async Task<int> ScrapeVolumeAsync(string volumeName, CancellationToken cancellationToken = default)
    {
        _logger.LogInformation("Scraping Spirit volume: {VolumeName}", volumeName);

        try
        {
            // Build index file URL
            var indexUrl = PdsBrowseUrlBuilder.BuildIndexUrl("spirit", volumeName);
    ...
    /// <summary>
    /// Scrape all Spirit volumes (PANCAM, NAVCAM, HAZCAM, MI, DESCENT)
    /// </summary>
    public async Task<int> ScrapeAllVolumesAsync(CancellationToken cancellationToken = default)
    {
        ...
        var volumes = PdsBrowseUrlBuilder.GetVolumesForRover("spirit");
        var totalInserted = 0;
        ...
        _logger.LogInformation(
            "Completed all Spirit volumes: {Total} total photos inserted",
            totalInserted);
        ...
        CancellationToken cancellationToken)
    {
        var rover = await _context.Rovers
            .Include(r => r.Cameras)
            .FirstOrDefaultAsync(r => r.Id == SpiritRoverId, cancellationToken);

        if (rover == null)
        {
            throw new InvalidOperationException($"Spirit rover (ID {SpiritRoverId}) not found in database");
        }
